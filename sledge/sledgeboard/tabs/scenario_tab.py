...
class ScenarioMetricScoreData:
    ...
    metric_aggregator_file_index: int
    planner_name: str
...
class ScenarioTimeSeriesData:
    experiment_index: int
    planner_name: str
    time_series_values: npt.NDArray[np.float64]
    time_series_timestamps: List[int]
    time_series_unit: str
    time_series_selected_frames: Optional[List[int]]
...
def _safe_is_nan(value: Any) -> bool:
    try:
...
class ScenarioTab(BaseTab):
    ...
    def __init__(
        ...
        map_objects = [
            'Lane', 'Intersection', 'Stop Line', 'Crosswalk', 'Walkway', 'Carpark', 'Lane Connector', 'Lane Line',
        ]
        ...
        self.plot_state_keys = [
            'x [m]', 'y [m]', 'heading [rad]', 'velocity_x [m/s]', 'velocity_y [m/s]', 'speed [m/s]',
            'acceleration_x [m/s^2]', 'acceleration_y [m/s^2]', 'acceleration [m/s^2]', 'steering_angle [rad]', 'yaw_rate [rad/s]',
        ]
    ...
    def _click_planner_checkbox_group(self, attr: Any) -> None:
        ...
        filtered_simulation_figures = [d for d in self._simulation_figure_data if d.planner_name in self.enable_planner_names]
        if not filtered_simulation_figures:
            ...
            simulation_layouts = gridplot(
                ...
                ncols=self.get_plot_cols(plot_width=self.simulation_figure_sizes[0], offset_width=scenario_tab_style['col_offset_width']),
    ...
    def _update_scenario_plot(self) -> None:
        ...
        logger.info(f"Rending scenario plot takes {time.perf_counter() - start_time:.4f} seconds.")
    ...
    def _init_selection(self) -> None:
        ...
        self._scenario_token_multi_choice.options = list(self._experiment_file_data.available_scenario_tokens.keys())
        self._update_planner_names()
            ...
            for metric_statistics_dataframe in metric_statistics_dataframes:
                ...
                for planner_name in planner_names:
                    ...
                    if time_series[time_series_headers[0]].iloc[0] is None:
                        continue
                    time_series_values = _sanitize_array(list(chain.from_iterable(time_series[metric_statistics_dataframe.time_series_values_column])))
                    ...
                    aggregated_time_series_data[metric_statistics_dataframe.metric_statistic_name].append(
                        ScenarioTimeSeriesData(
                            experiment_index=index,
                            planner_name=planner_name,
                            time_series_values=np.round(time_series_values, 4),
            ...
            for data in scenario_time_series_data:
                ...
                if metric_statistic_name not in time_series_figures:
                    time_series_figures[metric_statistic_name] = self._render_scalar_figure(
                        ...
                        x_axis_label='frame',
                        hover=HoverTool(tooltips=[("Frame", "@x"), ("Value", "@y{0.0000}"), ("Time_us", "@time_us"), ("Planner", "$name")]),
                ...
                data_source = ColumnDataSource(dict(x=timestamp_frames, y=_sanitize_array(data.time_series_values).tolist(), time_us=data.time_series_timestamps))
    ...
    def _render_scenario_metric_score_scatter(self, scatter_figure: Figure, scenario_metric_score_data: Dict[str, List[ScenarioMetricScoreData]]) -> None:
        ...
        for _, metric_score_data in scenario_metric_score_data.items():
            for score_data in metric_score_data:
                score = _safe_float(score_data.score)
                if score is None:
                    continue
                ...
                if data_source_index not in data_sources:
                    data_sources[data_source_index] = ScenarioMetricScoreDataSource(
                        xs=[], ys=[], planners=[], aggregators=[], experiments=[], fill_colors=[],
                        marker=self.get_scatter_sign(score_data.metric_aggregator_file_index), legend_label=legend_label,
                    )
                ...
                data_sources[data_source_index].ys.append(float(np.round(score, 4)))
                ...
                data_sources[data_source_index].experiments.append(experiment_name)
                data_sources[data_source_index].fill_colors.append(fill_color)
        for _, data_source in data_sources.items():
            if not data_source.ys:
                continue
            sources = ColumnDataSource(dict(
                xs=data_source.xs,
                ys=_sanitize_array(data_source.ys).tolist(),
    ...
    def _render_scenario_metric_score(self) -> Dict[str, Figure]:
        ...
        for scenario_metric_score_data in selected_scenario_metric_score:
            if scenario_metric_score_data.planner_name not in self.enable_planner_names:
                continue
            data[scenario_metric_score_data.metric_statistic_name].append(scenario_metric_score_data)
        ...
        for index in range(number_of_figures):
            ...
            selected_metric_names = metric_statistic_names[starting_index: starting_index + self._number_metrics_per_figure]
    ...
    def _render_grid_plot(self, figures: Dict[str, Figure], plot_width: int, legend: bool = True) -> LayoutDOM:
        ...
        for _, figure_plot in figures.items():
            if legend:
        ...
        return gridplot(figure_plot_list, ncols=self.get_plot_cols(plot_width=plot_width), toolbar_location="left")
    ...
    def _render_scenario_metric_layout(self, figure_data: Dict[str, Figure], default_div: Div, plot_width: int, legend: bool = True) -> column:
        ...
        return column(self._render_grid_plot(figures=figure_data, plot_width=plot_width, legend=legend))
    ...
    def _render_simulations(self) -> column:
        ...
        if not selected_keys:
            ...
            return column(self._default_simulation_div)
        hidden_glyph_names = [
            label
        ...
        return gridplot(simulation_figures, ncols=self.get_plot_cols(plot_width=self.simulation_figure_sizes[0], offset_width=scenario_tab_style['col_offset_width']), toolbar_location=None)
    ...
    def _render_ego_expert_state_glyph(self, ego_expert_plot_aggregated_states: scenario_ego_expert_state_figure_type, ego_expert_plot_colors: Dict[str, str]) -> column:
        ...
        for plot_state_key in self.plot_state_keys:
            ...
            fig = self._render_scalar_figure(title='', y_axis_label=plot_state_key, x_axis_label='frame', hover=hover, sizes=scenario_tab_style["ego_expert_state_figure_sizes"])
            fig.yaxis.formatter = BasicTickFormatter(use_scientific=False)
            ego_expert_state_figures[plot_state_key] = fig
        for planner_name, plot_states in ego_expert_plot_aggregated_states.items():
            color = ego_expert_plot_colors.get(planner_name, None) or None
            for plot_state_key, plot_state_values in plot_states.items():
                fig = ego_expert_state_figures[plot_state_key]
                clean_vals = np.round(_sanitize_array(plot_state_values), 2).tolist()
                data_source = ColumnDataSource(dict(x=list(range(len(clean_vals))), y=clean_vals))
                if self.expert_planner_key in planner_name:
                    fig.circle(x="x", y="y", name=planner_name, color=color, legend_label=planner_name, source=data_source, size=2)
                else:
                    fig.line(x="x", y="y", name=planner_name, color=color, legend_label=planner_name, source=data_source, line_width=1)
        return self._render_grid_plot(figures=ego_expert_state_figures, plot_width=scenario_tab_style["ego_expert_state_figure_sizes"][0], legend=True)
        ...
        for figure_data in simulation_figure_data:
            ...
            for planner_name, planner_states in ego_expert_states.items():
                ...
                for planner_state in planner_states:
                    for plot_state_key in self.plot_state_keys:
                        state_key_value = _safe_float(self._get_ego_expert_states(state_key=plot_state_key, ego_state=planner_state))
                        ego_expert_plot_aggregated_states[planner_name][plot_state_key].append(0.0 if state_key_value is None else state_key_value)
        return self._render_ego_expert_state_glyph(ego_expert_plot_aggregated_states=ego_expert_plot_aggregated_states, ego_expert_plot_colors=ego_expert_plot_colors)
