...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Edit sledge_raw scenes with the new 'sudden pedestrian crossing' semantics and evaluate them without diffusion."
    )
    ...
    parser.add_argument(
        "--save-visuals",
        action="store_true",
        help="Try to save edited_raster.png / edited_vector.png. Visualization failures will be reported but won't stop the main evaluation.",
    )
    return parser
    ...
    def run_batch(self) -> None:
        ...
        for index, scene_path in enumerate(scene_paths, start=1):
            ...
            try:
                ...
                print(
                    f"[{index}/{total}] done: {scene_path} | edited_alignment={row['edited_alignment']:.4f} | accepted={row['accepted']}"
                )
            except Exception as exc:
                error_payload = {
                    "scene_path": str(scene_path),
...
def main() -> None:
    ...
    evaluator = EditedSceneEvaluator(args)
    if args.input:
        evaluator.run_one(Path(args.input), Path(args.output), index=1)
        return
    evaluator.run_batch()
